...
def fourier_2D_kernel(images):
    """
        returns 2Dthe fourier transform of the image seen
    """
    fourier_2D_features = np.zeros_like(images)
    ...
    for i in range(images.shape[0]):
        ...
        fourier_2D_features[i,0:1024] = np.abs(np.fft.fft2(image_1)).reshape(1024)
        fourier_2D_features[i,1024:2048] = np.abs(np.fft.fft2(image_2)).reshape(1024)
        fourier_2D_features[i,2048:3072] = np.abs(np.fft.fft2(image_3)).reshape(1024)

    return fourier_2D_features
