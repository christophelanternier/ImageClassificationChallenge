...
def predict_with_SVM(train_features, train_labels, test_images, test_labels=None, lambdas=[0.01], filename='../data/Y_te_SVM.csv'):
    for _lambda in lambdas:
        t1 = datetime.now()
        alphas, bias = one_versus_all_SVM(train_features.T, train_labels, _lambda=_lambda)
        t2 = datetime.now()
        print 'model fitted. duration: ', t2 - t1
        prediction = predict_SVM(alphas, bias, train_features.T, test_features.T)
        t3 = datetime.now()
...
def predict_with_class_PCA(train_features, train_labels, test_features, test_labels=None, PCA_dimensions=range(10, 35, 2), filename='../data/Y_te_PCA.csv'):
    t1 = datetime.now()
    means, projection_basis = compute_class_PCA_linear_space(train_features, train_labels)
    t2 = datetime.now()
    ...
    for dimension in PCA_dimensions:
        t3 = datetime.now()
        predicted_labels = predict_with_class_PCA_projection(test_features, means, projection_basis, dimension)
        t4 = datetime.now()
...
train_features = first_scattering_kernel(train_images)
test_features = first_scattering_kernel(test_images)

predict_with_SVM(train_features, train_labels, test_features, test_labels, lambdas=[0.00003], filename='../data/Y_te_scattering_lambda_3_10-5.csv')
